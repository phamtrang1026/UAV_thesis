...
int crypto_aead_decrypt(uint8_t* m, uint32_t* mlen, uint8_t* c,
		uint32_t clen, uint8_t* ad,
		uint32_t adlen, uint8_t* npub,
						uint8_t* k, uint8_t* t) {

 // if (clen < CRYPTO_ABYTES) return -1;
 // *mlen = clen = clen - CRYPTO_ABYTES;
  return ascon_aead_decrypt(m, t, c, clen, ad, adlen, npub, k);
}
