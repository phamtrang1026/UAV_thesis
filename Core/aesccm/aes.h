...
// typedef unsigned int uint32_t;             // 32-bit uint32_t, change to "long" for 16-bit machines
...
void aes_encrypt(const uint8_t in[],             // 16 uint8_ts of plaintext
                 uint8_t out[],                  // 16 uint8_ts of ciphertext
                 const uint32_t key[],            // From the key setup
                 int keysize);                // Bit length of the key, 128, 192, or 256

void aes_decrypt(const uint8_t in[],             // 16 uint8_ts of ciphertext
                 uint8_t out[],                  // 16 uint8_ts of plaintext
                 const uint32_t key[],            // From the key setup
                 int keysize);                // Bit length of the key, 128, 192, or 256
...
int aes_encrypt_ccm(const uint8_t plaintext[],              // IN  - Plaintext.
                    ...
                    unsigned short associated_data_len,  // IN  - Associated Data length in uint8_ts.
                    const uint8_t nonce[],                  // IN  - The Nonce to be used for encryption.
                    unsigned short nonce_len,            // IN  - Nonce length in uint8_ts.
                    ...
                    uint8_t mac[16], uint32_t mac_len,                        // IN  - The desired length of the MAC, must be 4, 6, 8, 10, 12, 14, or 16.
...
int aes_decrypt_ccm(const uint8_t ciphertext[],             // IN  - Ciphertext, the concatination of encrypted plaintext and MAC.
                    uint32_t ciphertext_len,                 // IN  - Ciphertext length in uint8_ts.
                    const uint8_t assoc[],                  // IN  - The Associated Data, required for authentication.
                    unsigned short assoc_len,            // IN  - Associated Data length in uint8_ts.
                    ...
                    uint32_t *plaintext_len,                 // OUT - Length in uint8_ts of the output plaintext, always ciphertext_len - mac_len .
...
// XORs the in and out buffers, storing the result in out. Length is in uint8_ts.
...
// Input may be an arbitrary length (in uint8_ts).
...
int aes_encrypt_ccm(const uint8_t* payload, uint32_t payload_len, const uint8_t* assoc, unsigned short assoc_len,
                    const uint8_t* nonce, unsigned short nonce_len, uint8_t* out, uint32_t *out_len, uint8_t* mac,
                    uint32_t mac_len, const uint8_t* key_str, int keysize)
{
	uint8_t temp_iv[AES_BLOCK_SIZE], counter[AES_BLOCK_SIZE], *buf;
...
// Performs the Subuint8_ts step. All uint8_ts in the state are substituted with a
